...
const filteredData = (categoryBanner) => `
        <article class="category-cards">
            <img src="../..${categoryBanner.imageUrl}" alt="${
  categoryBanner.name
}" class="category-img">
            <div class="category-text">
                <h2 class="category-heading">${categoryBanner.name}</h2>
                <p><em><strong>${categoryBanner.description}</strong></em></p>
                <a href="../productsPage${anchorTagPage(
                  categoryBanner.key
                )}" class="app-btn btn-category">Explore ${
  categoryBanner.key
}</a>
            </div>
        </article>
